...
> 🔍 **Context Retrieval Priority:** Xem `_shared/context-retrieval.md` — Memory Service TRƯỚC, Logs SAU.

### 1.1. Load Order (Ưu tiên)

```
Step 0: 🧠 Query MCP Memory Service (LUÔN LÀM ĐẦU TIÊN)
├── recall_memory("today/yesterday/last week")  # Tìm theo thời gian
├── retrieve_memory("<project/topic>")           # Tìm theo nội dung
├── search_by_tag(["<project-tag>"])             # Tìm theo tags
└── Nếu có kết quả → Dùng làm context chính
    → Nếu đủ → Skip Step 3-4, nhảy Step 5
...
Step 5: Generate Summary (kết hợp Memory + .brain)
```
